...
import { changeUserPassword } from '../../auth/services/authService.js'
import { User, Save, Lock, KeyRound } from 'lucide-react'
import toast from 'react-hot-toast'

export default function MahasiswaSettings() {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      // In a real app, you might want to refresh userData in AuthContext, 
      // but since it reloads on refresh, it's fine for now or we can rely on local state.
    } catch (err) {
      ...
      <div className="glass-card" style={{ padding: '24px', marginBottom: '24px' }}>
        ...
        <form onSubmit={handleSaveProfile} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="form-group" style={{ marginBottom: 0 }}>
            ...
            <input type="email" value={userData?.email || user?.email || ''} disabled style={{ background: 'var(--clr-bg-tertiary)', color: 'var(--clr-text-muted)' }} />
          </div>
          <div className="form-group" style={{ marginBottom: 0 }}>
